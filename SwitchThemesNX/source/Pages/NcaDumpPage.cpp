...
void NcaDumpPage::Render(int X, int Y)
{
	...
	ImGui::TextWrapped("要安装 .nxtheme 文件，您需要先提取主菜单。\n"
		"每当固件更新或降级时都需要执行此操作。\n"
		"当提取的版本与您的固件不匹配时，系统会提示您进行此操作。\n\n"
		"通常您不需要手动提取，但如果遇到问题，可以在此尝试提取。");
	...
	if (ImGui::Button("提取主菜单"))
	{
		PushFunction([]() {
			...
			{
				DialogBlocking("输入了超级密钥组合，仅会导出主页菜单NCA（不会被提取）");
			...
			if (!YesNoPage::Ask(
				"要安装自定义主题，您需要先提取主菜单，此过程可能需要几分钟时间，请不要让您的主机进入睡眠模式，也不要按下Home键。\n"
				"是否继续？")) return;
...
void NcaDumpPage::CheckHomeMenuVer()
{
	...
	{
		DialogBlocking("要安装自定义主题，您需要先提取主菜单，此过程可能需要几分钟时间，请不要让您的主机进入睡眠模式，也不要按下Home键。\n按A键开始");
		goto DUMP_HOMEMENU;
	}
...
ASK_DUMP:
	if (!YesNoPage::Ask("当前固件版本与提取的主菜单版本不同，是否要重新提取主菜单？\n如果提取的主菜单与已安装的版本不匹配，主题将会崩溃。"))
	{
		DialogBlocking("您将不会再看到此消息，如果出现崩溃，您可以从主菜单中的`提取主菜单`选项手动提取主菜单");
		WriteHomeNcaVersion();
		return;
	}
